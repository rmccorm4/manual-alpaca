...
def order_list_all():
    ...
    print('{:>3} {:<7} {:>7} {:>7} {:>9}  {:>10}'.format('#','symbol','qty','side','status','client id'))

    for i in range(0,len(order_list)):
        print('{:>3} {:<7} {:>7} {:>7} {:>9}  {:>10}'.format(i,order_list[i]['symbol'],order_list[i]['qty'],order_list[i]['side'],order_list[i]['status'],order_list[i]['id']))
...
def order_list_open():
    ...
    print('{:>3} {:<7} {:>7} {:>7} {:>9}  {:>10}'.format('#','symbol','qty','side','status','client id'))
    for i in range(0,len(order_list)):
        print('{:>3} {:<7} {:>7} {:>7} {:>9}  {:>10}'.format(i,order_list[i]['symbol'],order_list[i]['qty'],order_list[i]['side'],order_list[i]['status'],order_list[i]['cid']))
    ...
    print('canceling {} order # {}'.format(order_list[inpint]['symbol'],order_list[inpint]['id']))
    try:
...
def get_positions():
    ...
    for p in positions:
        holdings[len] = {'symbol':p.symbol, 'qty':p.qty, 'current_price':p.current_price}
        if len in holdings:
            print('{:>3} {:<7} {:>7} {:>10}'.format(len, holdings[len]['symbol'], holdings[len]['qty'], holdings[len]['current_price']))
        len +=1
    ...
    if inp in holdings:
        ...
        selltype = input('selling {}: (M)arket, (L)imit: '.format(holdings[inp]['symbol'])).upper()
        
        sell_tif = input('time in force: (G)TC, (D)ay, (O)PG: ').upper()
        
        numsharesraw = input('# of shares to sell (max = {})'.format(holdings[inp]['qty']))
        ...
        if selltype == 'L':
            ...
            try:
                print('market order {} {} {}'.format(holdings[inp]['symbol'],numsharesraw,type_tif[sell_tif]))
                api.submit_order(
                    symbol=holdings[inp]['symbol'],
                    qty=numsharesraw,
                    side='sell',
